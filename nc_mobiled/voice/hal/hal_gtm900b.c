...
unsigned char hal_gtm900b_init (HAL_GTM900B *p_pstHAL)
{
	...
	memset (p_pstHAL, 0x00, sizeof (HAL_GTM900B));

	hal_gtm900b_get_ctrl_port_file (p_pstHAL, acDrvFile, sizeof (acDrvFile) - 1);
	...
	for (iPort = 0; HAL_GTM900B_MAX_DATA_PORT > iPort; iPort++)
	{
...
void hal_gtm900b_term (HAL_GTM900B *p_pstHAL)
{
	...
	{
		// Kill worker process
		hal_gtm900b_close (p_pstHAL);

		// Close all usable ports
		hal_gtm900b_port (p_pstHAL, 0x00, 0x00, 0x00);
...
{
	return thread_start (&p_pstHAL->stThread, hal_gtm900b_body);
}
...
unsigned char hal_gtm900b_attach (HAL_GTM900B *p_pstHAL, unsigned char p_ucPort, unsigned char p_ucPriority, unsigned char p_ucInterrupt,
								  HAL_GTM900B_NOTIFY p_pf_notify, void *p_pvPtr, int *p_piSID)
{
	unsigned char			ucRet;

	HAL_GTM900B_DATA_PORT	*pstDataPort	= 0;

	if (HAL_GTM900B_MAX_DATA_PORT < p_ucPort || 0 >= p_ucPort)
	{
		return 0x01;
	}
	...
	if (0 != pstDataPort->stSession.iSID)
	{
		...
		if (p_ucPriority > pstDataPort->stSession.ucPriority || (0x01 == p_ucInterrupt && p_ucPriority == pstDataPort->stSession.ucPriority))
		{
			...
			pstDataPort->stSession.pf_notify (pstDataPort->stSession.pvPtr, HAL_GTM900B_NOTIFY_REQ_DETACH, 0x00);
	...
	ucRet = hal_gtm900b_port (p_pstHAL, p_ucPort, 0x00, 0x01);
...
unsigned char hal_gtm900b_detach (HAL_GTM900B *p_pstHAL, int p_iSID)
{
	unsigned char			ucRet;
	...
	if (0 < p_iSID)
	{
		...
		// Close port
		hal_gtm900b_port (p_pstHAL, pstDataPort->ucPort, 0x00, 0x00);
...
unsigned char hal_gtm900b_request (HAL_GTM900B *p_pstHAL, int p_iSID, unsigned char p_ucOpID, ...)
{
  ...
  va_list               args;
	...
	else if (0 < p_iSID)
	{
		...
		for (iRet = 0; HAL_GTM900B_MAX_DATA_PORT > iRet; iRet++)
		{
	...
	case HAL_GTM900B_REQ_RESET:
		...
		hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_RESETTING_CODE, 0x00, __LINE__);
	...
	case HAL_GTM900B_REQ_DETACH:
		// Check all the holder priority with caller priority
		if (-1 != p_iSID)
		{
			for (ucRet = 0x00; HAL_GTM900B_MAX_DATA_PORT > ucRet; ucRet++)
			{
	...
	case HAL_GTM900B_REQ_CHG_SIM:
		...
		{
			if (0x00 != hal_gtm900b_sim_door (p_pstHAL, (unsigned char) iSim))
			{
			...
			hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_CHANGE_SIM_CODE, 0x00, __LINE__, (int) p_pstHAL->ucCurSIM, iSim);
			...
			*piRet = iSim;
			...
			for (iRet = 0; HAL_GTM900B_MAX_DATA_PORT > iRet; iRet++)
			{
				...
				{
					p_pstHAL->astDataPort [iRet].stSession.pf_notify (p_pstHAL->astDataPort [iRet].stSession.pvPtr, HAL_GTM900B_NOTIFY_SIM, 0x00, iSim);
				}
			}
		}
	...
	case HAL_GTM900B_REQ_OPEN_DATA_PORT:
		ucRet = hal_gtm900b_port (p_pstHAL, pstDataPort->ucPort, 0x00, 0x01);
		break;

	case HAL_GTM900B_REQ_CLOSE_DATA_PORT:
		ucRet = hal_gtm900b_port (p_pstHAL, pstDataPort->ucPort, 0x00, 0x00);
		break;

	case HAL_GTM900B_REQ_REG_FLAG:
	...
	case HAL_GTM900B_REQ_GET_DATA_PORT_FILE:
		pcRet = va_arg (args, char *);

		if (!(ucRet = hal_gtm900b_get_data_port_file (p_pstHAL, pstDataPort->ucPort, acBuf, sizeof (acBuf))))
      strcpy (pcRet, acBuf);
		break;

	case HAL_GTM900B_REQ_SET_SIM_AVAILABILITY:
	...
	if (0x01 == ucDetach)
	{
		...
		for (iRet = 0; HAL_GTM900B_MAX_DATA_PORT > iRet; iRet++)
		{
			if (0 != p_pstHAL->astDataPort [iRet].stSession.iSID)
			{
				p_pstHAL->astDataPort [iRet].stSession.pf_notify (p_pstHAL->astDataPort [iRet].stSession.pvPtr, HAL_GTM900B_NOTIFY_FORCED_DETACH, 0x00);
			}
		}
	}
...
unsigned char hal_gtm900b_at_cmd (HAL_GTM900B *p_pstHAL, int p_iSID, char *p_pcReq, char *p_pcRes, char *p_pcErr, char *p_pcRet, int p_iRetSize,
								  unsigned long p_ulDelay, unsigned long p_ulGap, unsigned long p_ulTimeout, unsigned char *p_pucProcToken, unsigned int p_uiLine, char *p_pcName)
{
	...
	if (0 != p_pcReq)
	{
		...
		if (0 != ucRet)
		{
			hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_DATA_PORT_FAILURE_CODE, ucRet, __LINE__);
			ucRet = 0x02;
			goto CleanUp;
		}
	}
...
CleanUp:
	...
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_DATA_PORT_RAW_CODE, ucRet, p_uiLine, p_pcName, acLog);

	return ucRet;
}

unsigned char hal_gtm900b_send (HAL_GTM900B *p_pstHAL, int p_iSID, unsigned char *p_pucData, int p_iLen)
{
	int						iIndex;
	...
	{
		for (iIndex = 0; HAL_GTM900B_MAX_DATA_PORT > iIndex; iIndex++)
		{
...
unsigned char hal_gtm900b_recv (HAL_GTM900B *p_pstHAL, int p_iSID, unsigned char *p_pucData, int p_iSize, int *p_piLen, unsigned long p_ulTimeout)
{
	int						iIndex;
...
unsigned char hal_gtm900b_switch_on (HAL_GTM900B *p_pstHAL, unsigned char p_ucSim, unsigned char *p_pucProcToken)
{
	...
	hal_gtm900b_select_sim (p_pstHAL, p_ucSim);
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_CURRENT_SIM_CODE, 0x00, __LINE__);

	// Open first ports with default settings [0]0000000
	ucRet = hal_gtm900b_port (p_pstHAL, 0x01, 0x00, 0x01);
	...
	if (0x01 == p_pstHAL->ucReset)
	{
		...
		hal_gtm900b_power_off (p_pstHAL);
		ISLEEP (15000);
		...
		hal_gtm900b_power_on (p_pstHAL);

		// Detect initial message if any
		ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, 0, "ready", "ERROR", 0, 0, 0, 0, 3000, p_pucProcToken, __LINE__, HAL_GTM900B_NAME);
		...
		ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, "AT\r", "OK	ready", "ERROR", acRet, sizeof (acRet) - 1, 1000, 0, 2000,
			p_pucProcToken, __LINE__, HAL_GTM900B_NAME);

		if (0x00 == ucRet)
		{
			ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, "ATH\r", "OK	NO CARRIER	ERROR", "", acRet, sizeof (acRet) - 1, 0, 0, 5000,
				p_pucProcToken, __LINE__, HAL_GTM900B_NAME);
	...
	{
		// Try AT command response
		ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, "AT\r", "OK	ready", "ERROR", 0, 0, 0, 0, 2000, p_pucProcToken, __LINE__, HAL_GTM900B_NAME);
	...
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_POWERING_ON_CODE, 0x00, __LINE__);
	hal_gtm900b_power_on (p_pstHAL);
	...
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_CHANGING_BAUDRATE_CODE, 0x00, __LINE__);
	...
	for (iIndex = 0; 2 > iIndex && 0x01 == *p_pucProcToken; iIndex++)
	{
		...
		ucRet = hal_gtm900b_port (p_pstHAL, 0x01, 0x80, 0x01);
		...
		sprintf (acIPR, "AT+IPR=%d\r", HAL_GTM900B_DEF_BAUDRATE);

		ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, acIPR, "OK	ready", "ERROR", 0, 0, 1000, 0, 2000, p_pucProcToken, __LINE__, HAL_GTM900B_NAME);

		if (0x00 == ucRet || 0x05 == ucRet)
		{
			hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_CHANGED_BAUDRATE_CODE, 0x00, __LINE__);
			goto CleanUp;
		}
		else if (0xFF == ucRet)
		{
	...
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_POWER_ON_FAILURE_CODE, ucRet, __LINE__);
	ucRet = 0x01;
...
CleanUp:
	...
	if (0x00 == ucRet)
	{
		hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_POWERED_ON_CODE, 0x00, __LINE__, p_ucSim);
	}
...
unsigned char hal_gtm900b_switch_off (HAL_GTM900B *p_pstHAL, unsigned char *p_pucProcToken)
{
	...
	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_POWERING_OFF_CODE, 0x00, __LINE__);
	...
	ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, "AT%MSO\r", "OK	down", "ERROR", 0, 0, 0, 0, 2000, p_pucProcToken, __LINE__, HAL_GTM900B_NAME);
	...
	hal_gtm900b_dtr (p_pstHAL, 0x00);	// DTR LOW

	hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_POWERED_OFF_CODE, 0x00, __LINE__);
	ucRet = 0x00;
...
unsigned char hal_gtm900b_get_ready (HAL_GTM900B *p_pstHAL)
{
	...
	hal_gtm900b_dtr (p_pstHAL, 0x01);	// DTR ON (LOW)

	ucRet = hal_gtm900b_at_cmd (p_pstHAL, -1, "AT&D1\r", "OK", "ERROR",
		0, 0, 0, 0, 2000, &p_pstHAL->stThread.ucProcToken, __LINE__, HAL_GTM900B_NAME);

	if (0x00 != ucRet)
	{
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Enable DTR is not ready. RetCode [0x%02X]", __LINE__, HAL_GTM900B_NAME, ucRet);
...
void hal_gtm900b_dtr (HAL_GTM900B *p_pstHAL, unsigned char p_ucFlag)
{
	if (0 < p_pstHAL->stCtrlPort.hComPort && 0x01 == p_pstHAL->ucReady)
	{
		...
		{
			hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_DTR_CODE, 0x01, __LINE__);
		...
		{
			hal_gtm900b_log (p_pstHAL, HAL_GTM900B_LOG_DTR_CODE, 0x00, __LINE__);
...
unsigned char hal_gtm900b_port (HAL_GTM900B *p_pstHAL, unsigned char p_ucPort, unsigned char p_ucSettings, unsigned char p_ucMode)
{
	...
	if (0x01 == p_ucPort || 0x00 == p_ucPort)
	{
		if (0x01 == p_ucMode)
		{
			ucRet = hal_gtm900b_get_data_port_file (p_pstHAL, 0x01, acDrvFile, sizeof (acDrvFile));
			...
			{
				comport_init_voice (&p_pstHAL->astDataPort [0].stPort, acDrvFile, HAL_GTM900B_FAC_BAUDRATE, HAL_GTM900B_FAC_DATA_BIT,
					HAL_GTM900B_FAC_STOP_BIT, HAL_GTM900B_FAC_PARITY, HAL_GTM900B_FAC_FLOW_CTRL, HAL_GTM900B_FAC_FRAG_SIZE, 0, 0, 0);
			}
			else
			{
				comport_init_voice (&p_pstHAL->astDataPort [0].stPort, acDrvFile, HAL_GTM900B_DEF_BAUDRATE, HAL_GTM900B_DEF_DATA_BIT,
					HAL_GTM900B_DEF_STOP_BIT, HAL_GTM900B_DEF_PARITY, HAL_GTM900B_DEF_FLOW_CTRL, HAL_GTM900B_DEF_FRAG_SIZE, 0, 0, 0);
			}
...
void hal_gtm900b_log (HAL_GTM900B *p_pstHAL, int p_iLogID, unsigned char p_ucRet, ...)
{
	...
	switch (p_iLogID)
	{
	case HAL_GTM900B_LOG_DATA_PORT_FAILURE_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Data port failure. RetCode [0x%02X]", uiLine, HAL_GTM900B_NAME, p_ucRet);
		break;

	case HAL_GTM900B_LOG_CURRENT_SIM_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module is using SIM %d", uiLine, HAL_GTM900B_NAME, p_pstHAL->ucCurSIM);
		break;

	case HAL_GTM900B_LOG_CHANGING_BAUDRATE_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Changing module baudrate setting", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_CHANGED_BAUDRATE_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Changed module baudrate setting", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_POWER_ON_FAILURE_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module power on failure. RetCode [0x%02X]", uiLine, HAL_GTM900B_NAME, p_ucRet);
		break;

	case HAL_GTM900B_LOG_POWERING_ON_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module powering on", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_POWERED_ON_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module powered on", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_POWERING_OFF_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module switching off", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_POWERED_OFF_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module switched off", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_RESETTING_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module resetting", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_RESET_CODE:
		logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module reset", uiLine, HAL_GTM900B_NAME);
		break;

	case HAL_GTM900B_LOG_CHANGE_SIM_CODE:
		iCurSim = va_arg (args, int);
		iNewSim = va_arg (args, int);

		if (iCurSim != iNewSim)
		{
			logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module change SIM%d -> SIM%d", uiLine, HAL_GTM900B_NAME, iCurSim, iNewSim);
		}
		else
		{
			logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Module change SIM%d", uiLine, HAL_GTM900B_NAME, iNewSim);
		}
		break;

	case HAL_GTM900B_LOG_DTR_CODE:
		logger_log (&g_stVoiceLog, LOG_DETAIL, "@%04d %s: DTR is %s", uiLine, HAL_GTM900B_NAME, (0x00 == p_ucRet) ? "LOW" : "HIGH");
		break;

	case HAL_GTM900B_LOG_DATA_PORT_RAW_CODE:
	...
	default:
		logger_log (&g_stVoiceLog, LOG_DETAIL, "@%04d %s: Event %d happen. RetCode [0x%02X]", uiLine, HAL_GTM900B_NAME, p_iLogID, p_ucRet);
	}
...
void hal_gtm900b_body (void *p_pvThread)
{
	...
	HAL_GTM900B			*pstHAL				= ((WRK_THREAD *) p_pvThread)->pvPtr;

	#if defined (ARCH_L200) || defined (ARCH_L350) || defined (ARCH_N300) || defined (ARCH_L300B)

		unsigned long	ulLastSIM_2			= 0;

	#endif

	pstHAL->ucSIM_1_Door = hal_gtm900b_sim_door (pstHAL, 0x01);
	pstHAL->ucGotSIM_1 = (0x00 == pstHAL->ucSIM_1_Door) ? 0x01 : 0x00;

	#if defined (ARCH_L200) || defined (ARCH_L350) || defined (ARCH_N300) || defined (ARCH_L300B)

		pstHAL->ucSIM_2_Door = hal_gtm900b_sim_door (pstHAL, 0x02);
	...
	while (0x01 == pstHAL->stThread.ucProcToken)
	{
		if (0x01 != pstHAL->ucReady && (0x01 == pstHAL->ucGotSIM_1 || 0x01 == pstHAL->ucGotSIM_2))
		{
			...
			if (0x01 == ucGo)
			{
				ucRet = hal_gtm900b_switch_on (pstHAL, pstHAL->ucCurSIM, &pstHAL->stThread.ucProcToken);

				if (0x00 == ucRet)
				{
					if (0x00 == hal_gtm900b_get_ready (pstHAL))
					{
						pstHAL->ucReady = 0x01;
						pstHAL->ucReset = 0x00;
		...
		ucRet = hal_gtm900b_sim_door (pstHAL, 0x01);
		...
		if (0x01 == ucRet)	// Now door is removed
		{
			if (0x00 == pstHAL->ucSIM_1_Door)	// Previous door is restored
			{
				...
				if (ulCurTime - ulLastSIM_1 >= 2000 && ulCurTime >= ulLastSIM_1 && 0 != ulLastSIM_1)
				{
					...
					{
						hal_gtm900b_request (pstHAL, -1, HAL_GTM900B_REQ_DETACH);
					}

					ulLastSIM_1 = 0;

					logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: SIM slot 1 is removed", __LINE__, HAL_GTM900B_NAME);
				}
		...
		else	// Now door is restored
		{
			if (0x01 == pstHAL->ucSIM_1_Door)	// Previous door is removed
			{
				...
				if (ulCurTime - ulLastSIM_1 >= 500 && ulCurTime >= ulLastSIM_1 && 0 != ulLastSIM_1)
				{
					...
					logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: SIM slot 1 is restored", __LINE__, HAL_GTM900B_NAME);
				}
				else if (ulCurTime < ulLastSIM_1 || 0 == ulLastSIM_1)
				{
					ulLastSIM_1 = ulCurTime;
			...
			ucRet = hal_gtm900b_sim_door (pstHAL, 0x02);
			...
			if (0x01 == ucRet)	// Now door is removed
			{
				if (0x00 == pstHAL->ucSIM_2_Door)	// Previous door is restored
				{
					...
					if (ulCurTime - ulLastSIM_2 >= 2000 && ulCurTime >= ulLastSIM_2 && 0 != ulLastSIM_2)
					{
						...
						logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: SIM slot 2 is removed", __LINE__, HAL_GTM900B_NAME);
					}
			...
			else	// Now door is restored
			{
				if (0x01 == pstHAL->ucSIM_2_Door)	// Previous door is removed
				{
					...
					if (ulCurTime - ulLastSIM_2 >= 500 && ulCurTime >= ulLastSIM_2 && 0 != ulLastSIM_2)
					{
						...
						logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: SIM slot 2 is restored", __LINE__, HAL_GTM900B_NAME);
					}
					else if (ulCurTime < ulLastSIM_2 || 0 == ulLastSIM_2)
					{
						ulLastSIM_2 = ulCurTime;
		...
		if ((0x00 == pstHAL->ucGotSIM_1 && 0x00 == pstHAL->ucGotSIM_2) || (0x01 == pstHAL->ucSIM_1_Door && 0x01 == pstHAL->ucSIM_2_Door))
		{
			if (0x00 == ucWaitSIM)
			{
				...
				hal_gtm900b_request (pstHAL, -1, HAL_GTM900B_REQ_DETACH);
				logger_log (&g_stVoiceLog, LOG_WARNING, "@%04d %s: Waiting for SIM insertion", __LINE__, HAL_GTM900B_NAME);

				hal_gtm900b_switch_off (pstHAL, &pstHAL->stThread.ucProcToken);
			}
		}
		else
		{
	...
	ucRet = 0x01;
	hal_gtm900b_switch_off (pstHAL, &ucRet);
}
