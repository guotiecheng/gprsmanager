...
{
	comport_term_voice (p_pstComPort);
	...
	p_pstComPort->pfFragEnd = p_pfFragEnd;
	p_pstComPort->pvBasket = p_pvBasket;

	p_pstComPort->iMagic = COM_PORT_VOICE_MAGIC;
	p_pstComPort->ucIsConnected = 0x00;
}

void comport_term_voice (COM_PORT_VOICE *p_pstComPort)
{
	comport_close_voice (p_pstComPort);
	memset (p_pstComPort, 0x00, sizeof (COM_PORT_VOICE));
}
...
unsigned char comport_open_voice (COM_PORT_VOICE *p_pstComPort)
{
	...
	if (COM_PORT_VOICE_MAGIC != p_pstComPort->iMagic)
	{
		ucRet = 0xFF;
		goto CleanUp;
	}
	...
	comport_close_voice (p_pstComPort);
		...
		if (0 < p_pstComPort->lBaudrate)
		{
			...
			{
				stNewCfg.StopBits = 0x00;	// Set the stop bit
			}
			else
			{
				stNewCfg.StopBits = 0x01;	// Set the stop bit
			}
		...
		if (0 >= p_pstComPort->lBaudrate)
		{
			...
			OPEN (p_pstComPort->hComPort, p_pstComPort->acDevFile, O_RDWR | O_NOCTTY | O_NONBLOCK | O_NDELAY);
...
*/
			...
			stNewCfg.c_cflag |= (CLOCAL | CREAD); /* remove comment by WENJING, or will open sc16c port fail */
...
unsigned char comport_send_voice (COM_PORT_VOICE *p_pstComPort, unsigned char *p_pucData, int p_iLen)
{
	...
	if (0x01 != p_pstComPort->ucIsConnected)
	{
		...
		if (p_pstComPort->iFragSize < p_iLen)
		{
			...
			{
        if (p_pstComPort->iFragSize < pucEnd - pucPtr)
          iLen = p_pstComPort->iFragSize;
        else
          iLen = pucEnd - pucPtr;

				iSend = write (p_pstComPort->hComPort, pucPtr, iLen);

				if (0 >= iSend || iLen != iSend)
				{
					ucRet = 0x03;
					goto CleanUp;
				}
        ...
        pucPtr += iLen;
				...
				// Wait until send fragment finished
				if (0 != p_pstComPort->pfFragDelay)
				{
					(p_pstComPort->pfFragDelay) (p_pstComPort->pvBasket, iLen);
				}
			}
			while (pucPtr < pucEnd);
		}
		else
		{
			...
			if (0 != p_pstComPort->pfFragDelay)
			{
				(p_pstComPort->pfFragDelay) (p_pstComPort->pvBasket, p_iLen);
			}
		}

		// Wait until send finished
		if (0 != p_pstComPort->pfFragEnd)
		{
			(p_pstComPort->pfFragEnd) (p_pstComPort->pvBasket);
		}
